...
GLuint QImguiWidget::CreateTexture(uint8_t *data, int w, int h, int fmt) {
    ...
    */
    QObject::connect(this->context(), &QOpenGLContext::aboutToBeDestroyed,
                     [gl, tex]() { gl->extraFunctions()->glDeleteTextures(1, &tex); });
...
*/
    QObject::connect(gl, &QOpenGLContext::aboutToBeDestroyed,
                     [gl, VaoHandle, ElementsHandle, VboHandle, FragHandle, VertHandle, ShaderHandle]() {
                         auto *funs = gl->extraFunctions();
...
void QImguiWidget::mouseDoubleClickEvent(QMouseEvent *event) { return QOpenGLWidget::mouseDoubleClickEvent(event); }
